...
Having trouble with Prowl? This guide covers the most common problems and how to resolve them.
...
- **Check your subscription status** — Scans only run for stores with an active subscription or active free trial. Go to **Settings** or **Billing** in the Prowl dashboard to verify your billing status.

### The scan completed but shows "Failed"

A failed scan means Prowl's browser couldn't fully load your product page. Common causes:
...
- **The page requires a password** — If your store is password-protected (common during setup), Prowl can't access it. Remove the password from **Online Store > Preferences** to enable scanning.
...
- **Prowl checks for specific issue types** — It looks for the problems described in [How Detection Works](how-detection-works.md). If the issue you're seeing isn't one of those types, it won't be flagged.
...
- **Check your alert settings** — Go to **Settings** in the Prowl dashboard and make sure email alerts are turned on.
...
- **Check that the app is still installed** — Go to **Settings > Apps and sales channels** in your Shopify admin to confirm Prowl is listed.
- **Verify required permissions** — Prowl needs permission to read your products and themes. If these permissions were revoked, you may need to reinstall the app.
...
- **Check if the page is publicly accessible** — Open the product page URL in an incognito/private browser window. If you can't access it, Prowl can't either.
- **Make sure the product is published** — Draft or archived products may not have publicly accessible pages.
- **Remove store password protection** — If your store has a password page enabled, Prowl cannot access any pages. Disable it in **Online Store > Preferences**.
